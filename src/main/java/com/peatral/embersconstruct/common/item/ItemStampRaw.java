...
public class ItemStampRaw extends ItemEmbersConstruct implements IMetaItem {
    ...
    public String getTexture(int meta) {
        return "stamp_" + EnumStamps.values()[meta].getName() + "_raw";
    }
    ...
    public String getUnlocalizedName(ItemStack item) {
        if (item.getItemDamage() <= EnumStamps.values().length - 1) {
            return "item.stamp_" + EnumStamps.values()[item.getItemDamage()].getName().toLowerCase(Locale.ROOT) + "_raw";
        }
        return "Invalid";
    }
}