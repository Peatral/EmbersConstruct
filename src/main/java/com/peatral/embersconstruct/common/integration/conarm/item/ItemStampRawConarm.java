...
public class ItemStampRawConarm extends ItemEmbersConstruct implements IMetaItem {
    ...
    public String getTexture(int meta) {
        return "stamp_" + EnumStampsConarm.values()[meta].getName() + "_raw";
    }
    ...
    public String getUnlocalizedName(ItemStack item) {
        if (item.getItemDamage() <= EnumStampsConarm.values().length - 1) {
            return "item.stamp_" + EnumStampsConarm.values()[item.getItemDamage()].getName().toLowerCase(Locale.ROOT) + "_raw";
        }
        return "Invalid";
    }
}